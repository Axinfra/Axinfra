...
export default function AdminSystemPage() {
  ...
  return (
    <div className="px-4 py-5 sm:px-6 sm:py-7 lg:px-9 lg:py-8 max-w-[1400px]">
      ...
      {/* Header */}
      <div className="mb-5">
        <h1 className="text-xl sm:text-2xl font-bold text-[var(--ax-text)]">System &amp; Issues</h1>
        <p className="text-[13px] text-[rgba(var(--ax-text-rgb),0.45)] mt-1">Open follow-ups, system events, and audit trail</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          <div key={label} className="bg-[var(--ax-surface)] border border-[var(--ax-border)] rounded-xl p-4">
            <div className="text-[10.5px] text-[rgba(var(--ax-text-rgb),0.4)] font-semibold uppercase tracking-wide mb-2">{label}</div>
            {value !== null
            ...
            {value !== null && <div className="text-[11px] text-[rgba(var(--ax-text-rgb),0.35)] mt-1">{sub}</div>}
          </div>
        ))}
      </div>

      {/* Tabs — scrollable on mobile */}
      <div className="flex gap-1 mb-5 bg-[var(--ax-surface)] border border-[var(--ax-border)] rounded-xl p-1 w-fit max-w-full overflow-x-auto">
        {tabs.map(({ key, label }) => (
      ...
      {loading ? (
        <div className="text-center py-16 text-[rgba(var(--ax-text-rgb),0.4)] text-sm">Loading system data…</div>
      ) : error ? (
        ...
        <div className="bg-[var(--ax-surface)] border border-[var(--ax-border)] rounded-xl overflow-hidden">
          ...
          {tab === 'followups' && (
            data.followUps.length === 0
              ...
              : <>
                  {/* Mobile cards */}
                  <div className="block sm:hidden divide-y divide-[var(--ax-border-subtle)]">
                    {data.followUps.map(f => {
                      ...
                      return (
                        <div key={f.id} className="p-4">
                          ...
                          <p className="text-[12.5px] text-[rgba(var(--ax-text-rgb),0.7)] mb-1.5">{f.description}</p>
                          <div className="text-[11px] text-[rgba(var(--ax-text-rgb),0.35)]">{f.project?.name ?? '—'} · {fmt(f.createdAt)}</div>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div className="hidden sm:block overflow-x-auto">
                    <table className="w-full border-collapse min-w-[580px]">
                      <thead><tr className="bg-[var(--ax-overlay)]">
                      ...
                      <tbody>
                        {data.followUps.map(f => {
                          const s = FU[f.status] ?? { bg: 'var(--ax-overlay)', fg: 'var(--ax-text)' };
                          return (
                            <tr key={f.id} className="border-t border-[var(--ax-border-subtle)] hover:bg-[var(--ax-overlay)]">
                              ...
                              <td className="px-4 py-3 text-[12.5px] text-[rgba(var(--ax-text-rgb),0.7)] max-w-[300px]">{f.description}</td>
                              <td className="px-4 py-3 text-[12.5px] text-[rgba(var(--ax-text-rgb),0.5)]">{f.project?.name ?? '—'}</td>
                              <td className="px-4 py-3 text-[11.5px] text-[rgba(var(--ax-text-rgb),0.35)] whitespace-nowrap">{fmt(f.createdAt)}</td>
                            </tr>
                          );
                        })}
                      </tbody>
          ...
          {tab === 'events' && (
            data.systemEvents.length === 0
              ? <div className="px-5 py-12 text-center text-[rgba(var(--ax-text-rgb),0.3)] text-sm">No events recorded</div>
              : <>
                  {/* Mobile */}
                  <div className="block sm:hidden divide-y divide-[var(--ax-border-subtle)]">
                      ...
                      return (
                        <div key={e.id} className="p-4">
                          <div className="flex items-center gap-2 mb-1.5">
                            ...
                            <span className="text-[12px] font-semibold text-[rgba(var(--ax-text-rgb),0.7)]">{fmtType(e.eventType)}</span>
                          </div>
                          <p className="text-[12.5px] text-[rgba(var(--ax-text-rgb),0.65)] mb-1.5">{e.message}</p>
                          <div className="text-[11px] text-[rgba(var(--ax-text-rgb),0.35)]">{e.actor?.name ?? '—'} · {e.project?.name ?? '—'} · {fmt(e.createdAt)}</div>
                        </div>
                      );
                    })}
                  </div>
                    ...
                    <table className="w-full border-collapse min-w-[620px]">
                      ...
                      <tbody>
                        {data.systemEvents.map(e => {
                          const sv = SEV[e.severity] ?? SEV['INFO'];
                          return (
                            <tr key={e.id} className="border-t border-[var(--ax-border-subtle)] hover:bg-[var(--ax-overlay)]">
                              ...
                              <td className="px-4 py-3 text-[12px] font-semibold text-[rgba(var(--ax-text-rgb),0.7)] whitespace-nowrap">{fmtType(e.eventType)}</td>
                              <td className="px-4 py-3 text-[12.5px] text-[rgba(var(--ax-text-rgb),0.65)] max-w-[280px]">{e.message}</td>
                              <td className="px-4 py-3 text-[12.5px] text-[rgba(var(--ax-text-rgb),0.5)]">{e.actor?.name ?? '—'}</td>
                              <td className="px-4 py-3 text-[12.5px] text-[rgba(var(--ax-text-rgb),0.5)]">{e.project?.name ?? '—'}</td>
                              <td className="px-4 py-3 text-[11.5px] text-[rgba(var(--ax-text-rgb),0.35)] whitespace-nowrap">{fmt(e.createdAt)}</td>
                            </tr>
                          );
                        })}
                      </tbody>
          ...
          {tab === 'audit' && (
            data.auditLogs.length === 0
              ? <div className="px-5 py-12 text-center text-[rgba(var(--ax-text-rgb),0.3)] text-sm">No audit logs</div>
              : <>
                  {/* Mobile */}
                  <div className="block sm:hidden divide-y divide-[var(--ax-border-subtle)]">
                    {data.auditLogs.map(a => (
                      <div key={a.id} className="p-4">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[13px] font-semibold text-[var(--ax-text)]">{a.actor?.name ?? '—'}</span>
                        ...
                        <div className="text-[12.5px] text-[rgba(var(--ax-text-rgb),0.6)] mb-1">
                          {fmtType(a.actionType)} · <span className="text-[rgba(var(--ax-text-rgb),0.45)]">{a.entityType}</span>
                        </div>
                        <div className="text-[11px] text-[rgba(var(--ax-text-rgb),0.35)]">{a.project?.name ?? '—'} · {fmt(a.createdAt)}</div>
                      </div>
                    ))}
                  </div>
                  {/* Desktop */}
                      ...
                      <tbody>
                        {data.auditLogs.map(a => (
                          <tr key={a.id} className="border-t border-[var(--ax-border-subtle)] hover:bg-[var(--ax-overlay)]">
                            ...
                            <td className="px-4 py-3 text-[12.5px] text-[rgba(var(--ax-text-rgb),0.65)]">{a.entityType}</td>
                            <td className="px-4 py-3">
                              <div className="text-[13px] text-[var(--ax-text)] font-medium">{a.actor?.name ?? '—'}</div>
                              <div className="text-[11px] text-[rgba(var(--ax-text-rgb),0.4)]">{a.actor?.email}</div>
                            </td>
                            <td className="px-4 py-3 text-[12px] text-[rgba(var(--ax-text-rgb),0.5)]">{a.role}</td>
                            <td className="px-4 py-3 text-[12.5px] text-[rgba(var(--ax-text-rgb),0.5)]">{a.project?.name ?? '—'}</td>
                            <td className="px-4 py-3 text-[11.5px] text-[rgba(var(--ax-text-rgb),0.35)] whitespace-nowrap">{fmt(a.createdAt)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
