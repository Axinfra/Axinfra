...
/** Top-level landing/picker page — mirrors documents/architecture's pattern. Reports itself
 * has no dedicated top-level route (it lives at /projects/[projectId]/reports); this page
      ...
      .then((data) => {
        if (data.success) {
          ...
          if (data.data.length === 1) {
            router.replace(`/projects/${data.data[0].id}/reports`);
          }
        }
      })
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            ...
            {projects.map((p) => (
              <button
                key={p.id}
                onClick={() => router.push(`/projects/${p.id}/reports`)}
