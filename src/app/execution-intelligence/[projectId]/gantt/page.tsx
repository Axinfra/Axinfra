...
type TimelineTrack = 'execution' | 'procurement';
...
export default function GanttPage() {
  ...
  const [track, setTrack]           = useState<TimelineTrack>('execution');
  ...
  // % Complete from the source file is their real completion signal instead. Same rule as
  // GanttChart's isMilestoneDone and scheduleMetrics.ts, so this count stays consistent with
  // the chart's own "done" bar coloring and with EI Analytics/the Milestones tab.
  const doneMs        = filteredMs.filter(m => m.state === 'VERIFIED' || m.state === 'CLOSED' || (m.percentComplete ?? 0) >= 100).length;
  ...
  return (
    <Layout>
      ...
      <div className="space-y-4">
        ...
        {/* ── Track toggle: Execution schedule vs. Procurement (purchase orders) ── */}
        <div className="flex items-center bg-[rgba(255,255,255,0.05)] rounded-xl p-1 gap-0.5 w-fit">
          {(['execution', 'procurement'] as TimelineTrack[]).map(t => (
