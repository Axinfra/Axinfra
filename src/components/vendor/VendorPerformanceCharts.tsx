...
import type { AnalyticsKPIs, SCurvePoint, DelayBucket, OnTimeTrend, PaymentCycleDays } from '@/lib/contexts/VendorPortalContext';
...
/** The vendor's own performance charts — KPI row + tabbed S-Curve/Delay/Payment-cycle/On-time
 * panel. Shared by the standalone /vendor/analytics page and the desktop project dashboard's
 * Reports tab so both render identically instead of drifting apart. */
export default function VendorPerformanceCharts({
  kpis, sCurve, delayHistogram, paymentCycleDays, onTimeTrend,
}: {
  kpis: AnalyticsKPIs;
  sCurve: SCurvePoint[];
    ...
    <div className="space-y-5">
      ...
      <div className="bg-[var(--ax-card)] border border-[var(--ax-border)] rounded-xl overflow-hidden">
        ...
        <div className="p-5">
          ...
          {activeTab === 'payment' && (
            <div>
              <div className="mb-4"><h3 className="text-sm font-semibold text-[var(--ax-text)]">Payment Cycle</h3><p className="text-xs text-[rgba(var(--ax-text-rgb),0.4)] mt-0.5">Days from submission to payment eligibility, per activity</p></div>
              <div className="grid sm:grid-cols-4 gap-4 mb-5">
                <div className="sm:col-span-1 flex flex-col items-center justify-center bg-[var(--ax-overlay)] border border-[var(--ax-border-subtle)] rounded-xl py-8 px-4 text-center">
                  ...
                  <p className="text-xs text-[rgba(var(--ax-text-rgb),0.3)] mt-1">
                    Across {paymentCycleDays.byMilestone.length} activit{paymentCycleDays.byMilestone.length === 1 ? 'y' : 'ies'}
                  </p>
                </div>
              ...
              <h4 className="text-[10.5px] font-semibold uppercase tracking-wider text-[rgba(var(--ax-text-rgb),0.4)] mb-2">By Activity</h4>
              {paymentCycleDays.byMilestone.length === 0 ? (
                <EmptyChart message="No activities have reached payment eligibility yet" />
              ) : (
                <ResponsiveContainer width="100%" height={Math.max(160, paymentCycleDays.byMilestone.length * 36)}>
                  <BarChart data={paymentCycleDays.byMilestone} layout="vertical" margin={{ top: 4, right: 24, bottom: 4, left: 8 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--ax-chart-line)" horizontal={false} />
                    <XAxis type="number" tick={{ fontSize: 10.5, fill: 'rgba(var(--ax-text-rgb),0.4)' }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v}d`} />
                    <YAxis type="category" dataKey="title" width={160} tick={{ fontSize: 11, fill: 'rgba(var(--ax-text-rgb),0.55)' }} axisLine={false} tickLine={false} />
                    <Tooltip {...DARK_TOOLTIP} formatter={(v: number) => [`${v} days`, 'Cycle time']} />
                    <Bar dataKey="days" name="Days" radius={[0, 4, 4, 0]} fill="rgba(var(--ax-accent-rgb),0.7)" />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </div>
