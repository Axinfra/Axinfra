...
import { Search, X, Filter, ChevronLeft, ChevronRight, AlertCircle, SearchX } from 'lucide-react';
import {
  useMilestoneSearch,
  type MilestoneSearchFilters as ActivitySearchFilters,
  type SearchMilestone as SearchActivityResult,
} from '@/hooks/useMilestoneSearch';
...
export default function ActivitySearch({ projectId, onSearchActive }: ActivitySearchProps) {
      ...
      <div className="relative flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search activities by name, status, vendor..."
        ...
        <div className="mt-4">
          ...
          {!isLoading && !error && results.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-[rgba(232,228,220,0.55)]">No activities match your search. Try different filters.</p>
            </div>
          )}
          ...
          {!isLoading && !error && results.length > 0 && (
            <>
              <div className="space-y-3">
                {results.map((activity: SearchActivityResult) => (
                  <div
                    key={activity.id}
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className="text-sm font-semibold text-[#e8e4dc] truncate">{activity.title}</h3>
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-[rgba(232,228,220,0.55)] mt-1">
                          ...
                          {activity.assignedVendor && (
                            <span>Vendor: {activity.assignedVendor.name}</span>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="mt-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-[rgba(232,228,220,0.55)]">{activity.completionPercentage}% complete</span>
                      </div>
                      <div className="w-full bg-[rgba(255,255,255,0.05)] rounded-full h-1.5">
                        <div
                          className={`h-1.5 rounded-full transition-all ${
                            activity.completionPercentage === 100 ? 'bg-[#5cba80]'
                            : activity.completionPercentage >= 50 ? 'bg-[var(--ax-accent)]'
